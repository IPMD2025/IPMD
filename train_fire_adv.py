...
def train(args, epoch, dataset, train_loader, model, ViT_model,clothes_classifier,
    ...
    for batch_idx, (imgs, pids, cloth_ids, camid, attrlabel,des,des_inv,des_cloth) in enumerate(tqdm(train_loader)):#
        ...
        # else:
            # img, pid, _ = data
        # imgs, pids, cloth_ids, camid, attrlabel,des,des_inv,des_cloth =data
        pos_mask = pid2clothes[pids]
        if use_gpu:
            imgs, pids, cloth_ids, attrlabel,pos_mask = imgs.cuda(), pids.cuda(), cloth_ids.cuda(), attrlabel.float().cuda(),pos_mask.cuda()
        # if use_gpu:
        ...
        # classifier.train()
        loss = 0
        features, outputs = model(imgs,ViT_model,des,des_inv,des_cloth,pids,cloth_ids)
        htri_loss_0 = metric_criterion(features[0], pids)
        htri_loss_1 = metric_criterion(features[1], pids)
        htri_loss_2 = metric_criterion(features[2], pids)
        class_loss = class_criterion(outputs[0], pids)
        # loss = class_loss + htri_loss_0 + htri_loss_1 + htri_loss_2 
        
        # if epoch >= 15 and epoch < 25:
        #     loss= class_loss + htri_loss_1 + htri_loss_2
            
            # feat_c = model.module.bn_f(features[1].detach())#features[:,:768]
        logits_c=clothes_classifier(features[3].detach())#features[:,:768]
        clothes_loss = criterion_clothes(logits_c, cloth_ids)
        optimizer_cc.zero_grad()
        clothes_loss.backward()
        optimizer_cc.step()
        # elif epoch >= 25 :  
        #     loss= class_loss + htri_loss_2
        ...
        # if epoch >=0:
        #     # feat_adv = model.module.bn_f(features[1])#features[:,:768]
        logits_adv=clothes_classifier(features[3])#features[:,:768]
        adv_loss = criterion_adv(logits_adv, cloth_ids, pos_mask)
        loss += class_loss + htri_loss_0 + htri_loss_1 + htri_loss_2 + adv_loss
        ...
        clothes_losses.update(clothes_loss.item(), pids.size(0))
        adv_losses.update(adv_loss.item(), pids.size(0))
        losses.update(loss.item(), pids.size(0))
    if args.print_train_info_epoch_freq != -1 and epoch % args.print_train_info_epoch_freq == 0:
        print('Epoch{0} Cls:{cls_loss.avg:.4f} Tri_0:{htri_losses_0.avg:.4f} '
        'Tri_1:{htri_losses_1.avg:.4f} Tri_2:{htri_losses_2.avg:.4f}  CCls:{CCls.avg:.4f} adv:{adv.avg:.4f} losses:{losses.avg:.4f} '.format(
        epoch, cls_loss=class_losses, htri_losses_0=htri_losses_0,
        htri_losses_1=htri_losses_1, htri_losses_2=htri_losses_2,CCls=clothes_losses, adv=adv_losses,losses=losses))
    #     print('Epoch{0} Cls:{cls_loss.avg:.4f} Tri_0:{htri_losses_0.avg:.4f} '
    #     'Tri_1:{htri_losses_1.avg:.4f} Tri_2:{htri_losses_2.avg:.4f}  losses:{losses.avg:.4f} '.format(
    # epoch, cls_loss=class_losses, htri_losses_0=htri_losses_0,
    # htri_losses_1=htri_losses_1, htri_losses_2=htri_losses_2,losses=losses))#n_fg:{num_fg_classes}, num_fg_classes=num_fg_classes
    ...
    #           'Tri_1:{htri_losses_1.avg:.4f}  losses:{losses.avg:.4f} '.format(
    #         epoch, cls_loss=class_losses,
    #         htri_losses_1=htri_losses_1, losses=losses))#n_fg:{num_fg_classes}, num_fg_classes=num_fg_classes
    #     elif epoch >= 30 : 
    #         
    #         
    #         print('Epoch{0} Cls:{cls_loss.avg:.4f} '
    #           'Tri_2:{htri_losses_2.avg:.4f}  losses:{losses.avg:.4f} '.format(
    #         epoch, cls_loss=class_losses, htri_losses_2=htri_losses_2,losses=losses))#n_fg:{num_fg_classes}, num_fg_classes=num_fg_classes 
    #     else:
    #         
    #         htri_losses_1.update(htri_loss_1.item(), pids.size(0))
    #         losses.update(loss.item(), pids.size(0))
