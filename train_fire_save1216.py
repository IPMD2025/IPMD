...
def train(args, epoch, dataset, train_loader, model, ViT_model,
          optimizer, scheduler, class_criterion, metric_criterion,triplet_hard_criterion, use_gpu):#classifier,
    ...
    for batch_idx, (imgs, pids, cloth_ids, camid, attrlabel,des,des_inv,des_cloth) in enumerate(tqdm(train_loader)):#
        ...
        # if epoch > fg_start_epoch:
        # triplet_loss = metric_criterion(feat, pids)
        ...
        htri_loss_3 = metric_criterion(features[3], pids)#triplet_hard_criterion
        # if epoch>10:
        #     loss += htri_loss_0 + htri_loss_1 + htri_loss_3
        # else:
        loss += htri_loss_0 + htri_loss_1 + htri_loss_3 - 0.1*htri_loss_2
    ...
    if args.print_train_info_epoch_freq != -1 and epoch % args.print_train_info_epoch_freq == 0:
        print('Epoch{0} Cls:{cls_loss.avg:.4f} Tri_0:{htri_losses_0.avg:.4f} '
              'Tri_1:{htri_losses_1.avg:.4f} Tri_2:{htri_losses_2.avg:.4f} Tri_3:{htri_losses_3.avg:.4f} losses:{losses.avg:.4f} '.format(
            epoch, cls_loss=class_losses, htri_losses_0=htri_losses_0,
            htri_losses_1=htri_losses_1, htri_losses_2=htri_losses_2,htri_losses_3=htri_losses_3,losses=losses))#n_fg:{num_fg_classes}, num_fg_classes=num_fg_classes
