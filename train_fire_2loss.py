...
def train(args, epoch, dataset, train_loader, model, ViT_model,
          optimizer, scheduler, class_criterion, metric_criterion, use_gpu):#classifier,, FFM_criterion
    ...
    for batch_idx, (imgs, pids, cloth_ids, camid, attrlabel,des,des_inv,des_cloth) in enumerate(tqdm(train_loader)):#
        ...
        loss += htri_loss_0 + htri_loss_1 #+ htri_loss_2
    ...
    if args.print_train_info_epoch_freq != -1 and epoch % args.print_train_info_epoch_freq == 0:
        print('Epoch{0} Cls:{cls_loss.avg:.4f} Tri_0:{htri_losses_0.avg:.4f} Tri_1:{htri_losses_1.avg:.4f}'
              'losses:{losses.avg:.4f} '.format(
            epoch, cls_loss=class_losses, htri_losses_0=htri_losses_0,htri_losses_1=htri_losses_1,losses=losses))#n_fg:{num_fg_classes}, num_fg_classes=num_fg_classes
