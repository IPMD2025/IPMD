...
def train(args, epoch, dataset, train_loader, model, ViT_model,
          optimizer, scheduler, class_criterion,clo_criterion, metric_criterion,triplet_hard_criterion, lamd,use_gpu):#classifier,,isflag
    ...
    for batch_idx, (imgs, pids, cloth_ids, camid, attrlabel,des,des_inv,des_cloth) in enumerate(tqdm(train_loader)):#
        ...
        if args.ablation=='no':
            ...
            clo_loss = clo_criterion(outputs[1], cloth_ids)
            htri_loss_3 = triplet_hard_criterion(features[3], pids)
            # if isflag:
            #     loss += htri_loss_0 + htri_loss_1 - lamd*htri_loss_2 + htri_loss_3#
            # else:
            loss +=  htri_loss_0 + htri_loss_1 - lamd*(htri_loss_2+clo_loss)/2 + htri_loss_3#
        ...
        clo_losses.update(clo_loss.item(), pids.size(0))
    ...
    if args.print_train_info_epoch_freq != -1 and epoch % args.print_train_info_epoch_freq == 0:
        print('Epoch{0} Cls:{cls_loss.avg:.4f} Tri_0:{htri_losses_0.avg:.4f} '
              'Tri_1:{htri_losses_1.avg:.4f} Tri_2:{htri_losses_2.avg:.4f} Clo:{clo_loss.avg:.4f} Tri_3:{htri_losses_3.avg:.4f} losses:{losses.avg:.4f} lamd:{lamd:.4f}'.format(
            epoch, cls_loss=class_losses, htri_losses_0=htri_losses_0,
            htri_losses_1=htri_losses_1, htri_losses_2=htri_losses_2,clo_loss=clo_losses,htri_losses_3=htri_losses_3,losses=losses,lamd=lamd))#n_fg:{num_fg_classes}, num_fg_classes=num_fg_classes
