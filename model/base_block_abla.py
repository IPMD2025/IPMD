...
class TransformerClassifier(nn.Module):
    ...
    def forward(self, imgs,ViT_model,des=None,gt_label=None):#,group_arr
        ...
        feat_g=self.head(features[:,0])#img_ft[:,0]
       ...
       ###########################
        ...
        # att_vit_cls=torch.cat((att_vit_cls[:,:,0].unsqueeze(-1),att_vit_cls[:,:,1+self.lmbd:]),dim=-1)#+self.lmbd+ P
        ...
        # vis_embed = feat_cam#torch.cat((img_ft[:,:1+self.lmbd],),dim=1)#img_ft[:,1+self.lmbd:]#global_cam[:,0].unsqueeze(1)#+ self.pos_embed #+ self.vis_embed #features_cls_temp #features[:,5:]
    ...
    #     self.image_mask[1][C+P+N//2:].fill_(float("-inf"))   #0-hair， 1th，2th，3th块保留  1-age whole attention 2-gender whole attention
    #     self.image_mask[2][C+P+N//2:].fill_(float("-inf"))
    #     self.image_mask[3][:C+P+N//2].fill_(float("-inf"))  #4-accessory 1,2,3,4,5,6块保留 [6*2*14+8]  
    #     self.image_mask[4][:C+P+N//2].fill_(float("-inf"))
    #     self.image_mask[5][:C+P+1*N//4].fill_(float("-inf"))
    #     self.image_mask[5][C+P+3*N//4:].fill_(float("-inf"))   #3-carry 3,4,5,6块保留 [2*2*14+8,6*2*14+8]
