...
class TransformerClassifier(nn.Module):
    def __init__(self, num_classes,attr_num=35,attr_words='attribute', dim=768, pretrain_path='/media/backup/**/pretrained/jx_vit_base_p16_224-80ecf9dd.pth',**kwargs):#
        ...
        if args.ablation=='no':
            ...
            # self.bn_im=nn.BatchNorm1d(dim)
            # self.bn_im.bias.requires_grad_(False)
            # self.bn_im.apply(self._init_kaiming)
    ...
    def forward(self, imgs,ViT_model,des,des_inv,des_cloth,pids,cloth_ids,ViT_model_m=None,mask=None):#
        ...
        if args.ablation=='no':
            ...
            destoken_clo = clip.tokenize(des_cloth,truncate=True).cuda()
            # word_embed_clo_m=ViT_model_m.encode_text(destoken_clo).cuda().float()
            word_embed_clo=ViT_model.encode_text(destoken_clo).cuda().float()
            if mask is not None:
                ...
                feat_itmc=self.bn_mc(features_m[:,0])#word_embed_clo),dim=1)
        ...
        if self.training:
            if args.ablation =="no":
                ...
                return [feat_itc,feat_itmc,feat_tm,feat_f,feat_m,feat],logits
