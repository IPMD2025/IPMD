...
class TransformerClassifier(nn.Module):
    def __init__(self, num_classes,attr_num=35,attr_words='attribute', dim=768, pretrain_path='/media/backup/**/pretrained/jx_vit_base_p16_224-80ecf9dd.pth',**kwargs):#
        ...
        self.head_c = nn.Linear(dim*2, num_classes*2,bias=False)#nn.Conv1d(self.dim, self.attr_num, kernel_size=3, stride=1, padding=1)
        self.head_c.apply(self._init_weights)
        ...
        if args.ablation=='no':
            ...
            self.bn_d=nn.BatchNorm1d(2*dim)
            self.bn_d.bias.requires_grad_(False)
            self.bn_d.apply(self._init_kaiming)
    ...
    def forward(self, imgs,ViT_model,des,des_inv,des_cloth,pids,cloth_ids):
        ...
        if args.ablation=='no':
            ...
            logits_c=self.head_c(self.bn_d(feat_itc))
