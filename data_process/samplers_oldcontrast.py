...
class RandomIdentitySampler_cc(Sampler):
    ...
    def __iter__(self):
        ...
        for pid in self.pids:
            ...
            if len(idxs) < self.num_instances:
                idxs = np.random.choice(idxs, size=self.num_instances, replace=True)
            if len(idxs_clo[0]) < self.num_instances/2:
                idxs_clo[0] = np.random.choice(idxs_clo[0], size=self.num_instances/2, replace=True)
            if len(idxs_clo[1]) < self.num_instances/2:
                idxs_clo[1] = np.random.choice(idxs_clo[1], size=self.num_instances/2, replace=True)
            random.shuffle(idxs)
            random.shuffle(idxs_clo[0])
            random.shuffle(idxs_clo[1])
            batch_idxs = []
            for idx in range(min(len(idxs_clo[0]),len(idxs_clo[1]))):
