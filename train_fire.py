...
def train(args, epoch, dataset, train_loader, model, ViT_model,ViT_model_m,
    ...
    for batch_idx, (imgs, pids, cloth_ids, camid, attrlabel,des,des_inv,des_cloth,mask) in enumerate(tqdm(train_loader)):#
        ...
        if use_gpu:
            imgs, pids, cloth_ids = imgs.cuda(), pids.cuda(), cloth_ids.cuda()#, attrlabel,, attrlabel.float().cuda()
        ...
        if args.ablation=='no':
            ...
            htri_loss_0 = metric_criterion(features[0], cloth_ids)
            ...
            # conloss = triplet_hard_criterion(torch.cat((features[4],features[5]),dim=1), pids)
            conloss = con_loss(features[4], features[5], pids)
            ...
            loss += htri_loss_1 + htri_loss_3 - lamd*htri_loss_0- gama*htri_loss_2 + lamd*conloss+ class_loss_m #gama
            # htri_loss_2=torch.tensor(100.).cuda()
            # class_loss_m=torch.tensor(100.).cuda()
