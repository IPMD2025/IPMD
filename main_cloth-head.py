...
def main():
    ...
    # clo_criterion = CrossEntropyLabelSmooth(num_classes=num_classes*2, epsilon=0.1, use_gpu=use_gpu)
    clo_criterion = CrossEntropyLoss(num_classes=num_classes*2, use_gpu=use_gpu, label_smooth=False)
    ...
    triplet_hard_criterion = TripletLoss_hard(margin=args.margin)
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        clo_losses = train_fire.train(args, epoch + 1, dataset, train_loader, model,ViT_model, 
                         optimizer, scheduler, class_criterion,clo_criterion, metric_criterion, triplet_hard_criterion,lamd,use_gpu)#classifier,, FFM_criterion
        train_time += round(time.time() - start_train_time)
        if isflag:
            if clo_losses <= loss_before:
                loss_before = clo_losses
            else:
                lamd=0.1
                isflag=False
        # evaluate
