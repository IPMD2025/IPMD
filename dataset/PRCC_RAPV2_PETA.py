...
class PRCC(object):
    ...
    def __init__(self, dataset_root='data', dataset_filename='prcc', verbose=True, **kwargs):
        ...
        self.meta_dir_PETA='PAR_PETA_1219.txt'
        self._check_before_run()
        self.meta_dims_PETA=35
        imgdir2attribute_PETA = {}
        
        with open(osp.join(dataset_root, 'prcc',self.meta_dir_PETA), 'r') as f:
            for line in f:
                imgdir, attribute_id, is_present = line.split()
                if imgdir not in imgdir2attribute_PETA:
                    imgdir2attribute_PETA[imgdir] = [0 for i in range(self.meta_dims_PETA)]
                imgdir2attribute_PETA[imgdir][int(attribute_id)] = int(is_present) 

        train, num_train_pids, num_train_imgs, num_train_clothes, pid2clothes = \
            self._process_dir_train(self.train_dir, imgdir2attribute,imgdir2attribute_PETA)
        val, num_val_pids, num_val_imgs, num_val_clothes, _ = \
            self._process_dir_train(self.val_dir,imgdir2attribute,imgdir2attribute_PETA)

        query_same, query_diff, gallery, num_test_pids, \
        num_query_imgs_same, num_query_imgs_diff, num_gallery_imgs, \
        num_test_clothes, gallery_idx = self._process_dir_test(self.test_dir,imgdir2attribute,imgdir2attribute_PETA)
    ...
    def _process_dir_train(self, dir_path,imgdir2attribute,imgdir2attribute_PETA):
            ...
            for img_dir in img_dirs:
                ...
                for i in range(3): 
                    if imgdir2attribute[img_dir][i]==1: 
                        ...
                        description += hair[i]  
                        hairind += 1
                if imgdir2attribute[img_dir][0]==0 and imgdir2attribute[img_dir][1]==0:
                    if imgdir2attribute[img_dir][2]==1:
                        description += ", "
                ...
                #PETA
                # if imgdir2attribute_PETA[img_dir][5]==1:
                #     description += ", casual upper wear"
                #     des_cloth += ", casual upper wear"
                # if imgdir2attribute_PETA[img_dir][6]==1:
                #     description += ", formal upper wear"
                #     des_cloth += ", formal upper wear"
                
                if imgdir2attribute_PETA[img_dir][8]==1:
                    description += ", logo"
                    des_cloth += ", logo"
                if imgdir2attribute_PETA[img_dir][11]==1:
                    description += ", thin stripes"
                    des_cloth += ", thin stripes"
                #######    
    ...
    def _process_dir_test(self, test_path,imgdir2attribute,imgdir2attribute_PETA):
